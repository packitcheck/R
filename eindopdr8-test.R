...
df_interesse <- data.frame(AangegevenMaxSnelheid = ongevallen_dh_2006_2015$AangegevenMaxSnelheid,
                           ...
                           Datum = ongevallen_dh_2006_2015$Datum, 
...
#Groepeer interessante data
gegrouppeerde_interessante_data <- df_interesse %>%
  group_by(Longitude) #%>%
  #summarize(DAY.MAX = round(max(OngevalID), 3),
   #         DAY.MIN = round(min(OngevalID), 3),
    #        DAY.AVG = round(mean(OngevalID), 3))

View(gegrouppeerde_interessante_data)
