...
dag_max_DodelijkSlachtoffers <- ongevallen_dh_2006_2015 %>% group_by(Jaar) %>%
  group_by(Dag) %>% summarise((count = n()))
   
    
#Visualiseer locatie omgelukken
map_dh <- get_map("The Hague, The Netherlands", zoom = 12)
map_dh <- ggmap(map_dh)
plaats_ongeval <- map_dh + geom_point(data = ongevallen_dh_2006_2015, aes(x = Longitude, y = Latitude))
plaats_ongeval
